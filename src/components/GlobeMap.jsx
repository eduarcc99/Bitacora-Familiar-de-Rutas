...
const districtBorderPaint = {
  default: {
    ...
    'line-width': ['interpolate', ['linear'], ['zoom'], 10, 1.2, 13, 1.6, 16, 2],
  },
  focus: {
    'line-color': '#ffffff',
    'line-width': ['interpolate', ['linear'], ['zoom'], 7.5, 1.6, 10, 2, 13, 2.4],
  },
}
...
function syncProjection(map, provinceFilterId) {
  ...
  const useGlobe =
    map.getZoom() <= GLOBE_MAX_ZOOM && !provinceFilterId
...
function syncAdminLevelVisibility(map, provinceFilterId) {
  ...
  if (focus) {
    ...
    setLayerVisibility(map, ['peru-districts-fill', 'peru-districts-border'], showDistricts)
    ...
    if (map.getLayer('peru-districts-labels')) {
      applyWatermarkToLayer(map, 'peru-districts-labels', watermarkLabels.distFocus)
    }
  ...
  if (map.getLayer('peru-provinces-border') && showProv) {
    map.setPaintProperty('peru-provinces-border', 'line-color', provinceBorderPaint['line-color'])
    map.setPaintProperty('peru-provinces-border', 'line-width', provinceBorderPaint['line-width'])
    map.setPaintProperty('peru-provinces-border', 'line-opacity', provinceBorderPaint['line-opacity'])
  }
  setLayerVisibility(
    map,
...
export default function GlobeMap({
  ...
  const handleRegionClick = useCallback(
    (e) => {
      ...
      map.fitBounds(featureBounds(feature.geometry), { padding: 60, duration: 1400 })
      afterClickNav(map)
    },
    [afterClickNav]
  )
  ...
  const refreshDistrictSource = useCallback(async (map) => {
  ...
  const autoDetectAdminContext = useCallback((map) => {
    ...
    if (zoom >= AMAZONAS_PROVINCE_AUTO_ZOOM) {
      ...
      if (dept === 'AMAZONAS' && isAmazonasProvinceId(provinceFilterRef.current)) {
        departmentFilterRef.current = 'AMAZONAS'
        return
      }
      provinceFilterRef.current = null
      departmentFilterRef.current =
        dept === 'AMAZONAS' ? 'AMAZONAS' : zoom >= ZOOM_BAND.provMin ? dept : null
      return
    }
  ...
  const ensureDistrictLayers = useCallback(async (map, force = false) => {
    const filter = provinceFilterRef.current
  ...
  const updateDistrictData = useCallback(async (map) => {
    await refreshDistrictSource(map)
