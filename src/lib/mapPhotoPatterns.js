import { buildCollageImageData, urlsForMapCollage } from "./photoCollage";

const patternCache = new Map();
/** Sufijo para invalidar patrones viejos (1×1 / grids variables) */
const PATTERN_VERSION = "2x2";

export async function syncGeoJSONPhotoPatterns(
  map,
  geojson,
  sourceId,
  layerIds,
  ...
  for (const feature of geojson.features) {
    ...
    const collageUrls = urlsForMapCollage(urls);
    if (!collageUrls.length) continue;

    const patternId = `pat-${PATTERN_VERSION}-${sourceId}-${feature.id}`;
    ...
    try {
      ...
      if (!imageData) {
        imageData = await buildCollageImageData(collageUrls);
