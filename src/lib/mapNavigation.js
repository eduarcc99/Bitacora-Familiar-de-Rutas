...
import { getProvinceSlugById } from "../data/provincePlaces";
import {
  boundsFromGeoJSON,
...
export function fitMapToScopedBounds(map, bounds, options = {}) {
  ...
  let zoomOutBias = options.zoomOutBias ?? (options.district ? 0.85 : 1.15);
  ...
  const targetZoom = Math.max(
    options.minFitZoom ?? SCOPED_MIN_ZOOM,
...
export function filterToAdminRefs(filter = EMPTY_FILTER, places = []) {
  ...
  if (filter.district) {
    const district = resolveCatalogPlace(places, filter.district);
    // Amazonas: default 0101 si falta province_id (comportamiento actual)
    provinceFilterRef =
      district?.province_id ??
      (filter.region === "amazonas" ? "0101" : null);
    if (filter.region === "amazonas") {
      ...
      provinceFilterRef &&
      String(provinceFilterRef).padStart(4, "0").startsWith("01")
    ) {
      departmentFilterRef = "AMAZONAS";
    }
...
export function filterFromProvinceFeature(places, feature) {
  ...
  // Amazonas (y provincias en places.js): misma resolución de siempre
  const place = places.find((p) => p.province_id === provId);
  if (place) return filterFromSlug(places, place.slug);

  // Resto del Perú: catálogo prov-{IDPROV} (sin alterar Amazonas)
  const catalogProvinceSlug = getProvinceSlugById(provId);
  if (catalogProvinceSlug) {
    return filterFromSlug(places, catalogProvinceSlug);
  }

  const deptName = feature.properties.FIRST_NOMB || feature.properties.NOMBDEP;
